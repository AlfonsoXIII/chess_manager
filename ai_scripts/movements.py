...
def King_Castling(board, h_moved, colour, pos):
    local_castling = (False, False)
    k = 1 #(1 if self.fliped == False else -1)

    if h_moved == False and 0 <= pos[1]+(2*k) <= 7:
        f_board_1 = deepcopy(board)
        f_board_1[pos[0]][pos[1]+(1*k)] = "K" if colour == 0 else "k"
        f_board_1[pos[0]][pos[1]] = ""

        f_board_2 = deepcopy(board)
        f_board_2[pos[0]][pos[1]+(2*k)] = "K" if colour == 0 else "k"
        f_board_2[pos[0]][pos[1]] = ""

        if (board[pos[0]][pos[1]+(1*k)] == "" and King.Check(self, f_board_1, (pos[0], pos[1]+(1*k)))) and (board[pos[0]][pos[1]+(2*k)] == "" and King.Check(self, f_board_2, (pos[0], pos[1]+(2*k)))) and board[pos[0]][pos[1]+(3*k)] == ("R" if board[pos[0]][pos[1]].isupper() else "r"):
            local_castling = (True, local_castling[1])
    
    if h_moved == False and 0 <= pos[1]+(-3*k) <= 7:
        f_board = deepcopy(board)
        f_board[pos[0]][pos[1]-1*k] = "K" if colour == 0 else "k"
        f_board[pos[0]][pos[1]] = ""

        f_board_1 = deepcopy(board)
        f_board_1[pos[0]][pos[1]-2*k] = ("K" if colour == 0 else "k")
        f_board_1[pos[0]][pos[1]] = ""

        f_board_2 = deepcopy(board)
        f_board_2[pos[0]][pos[1]-3*k] = "K" if colour == 0 else "k"
        f_board_2[pos[0]][pos[1]] = ""

        if (board[pos[0]][pos[1]-1*k] == "" and King.Check(self, f_board, (pos[0], pos[1]-1*k))) and (board[pos[0]][pos[1]-2*k] == "" and King.Check(self, f_board_1, (pos[0], pos[1]-2*k))) and (board[pos[0]][pos[1]-3*k] == "" and King.Check(self, f_board_2, (pos[0], pos[1]-3*k))) and board[pos[0]][pos[1]-4*k] == ("R" if board[pos[0]][pos[1]].isupper() else "r"):
