...
class Text(): #Classe de l'historial de jugades
    ...
    def draw(self, jugada, text_data, fps, center):
        ...
        pygame.draw.rect(self.screen, (220, 220, 220),[int(380*self.proportion)+center,int(115*self.proportion),int(295*self.proportion), (math.trunc(len(text_data)/6)+1)*int(30*self.proportion)])
        pygame.draw.rect(self.screen,(189,189,189),[int(377*self.proportion)+center, int(112*self.proportion),int(301*self.proportion),6+((math.trunc(len(text_data)/6)+1)*int(30*self.proportion))],int(3*self.proportion), border_radius=int(10*self.proportion))
        ...
        for x in text_data:
            ...
            if jugada >= 0 and text_data.index(x)+1 == jugada:
                #Indicador circular
                pygame.draw.rect(self.screen, (0, 0, 0), ((390*self.proportion)+center+text_pos,
                                (30*self.proportion)*text_period+(94*self.proportion),
                                arial.size(str(x[0]))[0],
                                20*self.proportion))

                #Text
                self.screen.blit(arial.render(x[0], True, (255, 255, 255)),((390*self.proportion)+center+text_pos,
                                (30*self.proportion)*text_period+(94*self.proportion)))
        
            else:
                #Text
                self.screen.blit(arial.render(x[0], True, (0, 0, 0)),((390*self.proportion)+center+text_pos,
                                (30*self.proportion)*text_period+(94*self.proportion)))
