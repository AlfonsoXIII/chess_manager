...
class Menu():
    ...
    def Generate_Buttons(self): #Funció per a crear els botons i afegir-los a la finestra
        ...
        menu = pieces.Button("images/tirante.png", (0, 0, 141, 347), (0, 0, 141, 347), (int(30*self.proportion), 
                                                                                        int(110*self.proportion)), 
                                                                                        4)
        menu.rect.x = int(20*self.proportion)
        menu.rect.y = int(-6*self.proportion)

        arrow = pieces.Arrow_Button("images/menu_flecha.png", (0, 0, 105, 96), (int(20*self.proportion), 
                                                                                int(20*self.proportion)), 
                                                                                5, 
                                                                                self.proportion)
        arrow.rect.x = int(25*self.proportion)
        arrow.rect.y = int(82*self.proportion)

        config = pieces.Button("images/menu_pressed.bmp", (0, 146.5, 150, 293), (0, 0, 150, 143), (int(65*self.proportion), 
                                                                                                    int(65*self.proportion)), 
                                                                                                    6)
